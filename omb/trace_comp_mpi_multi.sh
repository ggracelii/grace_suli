...
run_composition () {
    ...
    echo -e "\n\nRunning Composition $comp (${label})..." >> mpi_multi_output.log
    ...
    stdbuf -o0 mpiexec -n $NUM_PROCS -ppn $PPN -hostfile hosts.txt \
        ...
        "$BIN" -m 4:4 -i 1 -x 0 >> mpi_multi_output.log 2>&1
}

run_dc_none () {
    local label="dc-none"
    echo "Running Device Collectives None..."
    echo "Running Device Collectives None..." > mpi_multi_output.log
...
sed -i '/^\([0-9]\|#\)/d' mpi_multi_output.log
